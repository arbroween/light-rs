...
unsafe extern "C" fn f_set_clip_rect(state: *mut lua_State) -> libc::c_int {
    ...
    rect.x = luaL_checknumber(state, 1) as libc::c_int;
    rect.y = luaL_checknumber(state, 2) as libc::c_int;
    rect.width = luaL_checknumber(state, 3) as libc::c_int;
    rect.height = luaL_checknumber(state, 4) as libc::c_int;
...
unsafe extern "C" fn f_draw_text(state: *mut lua_State) -> libc::c_int {
    let font = luaL_checkudata(state, 1, b"Font\0" as *const u8 as *const libc::c_char)
        as *mut *mut RenFont;
    let text: *const libc::c_char = luaL_checklstring(state, 2, ptr::null_mut());
    let mut x = luaL_checknumber(state, 3) as libc::c_int;
    let y = luaL_checknumber(state, 4) as libc::c_int;
    let color = checkcolor(state, 5, 255);
...
pub unsafe extern "C" fn luaopen_renderer(state: *mut lua_State) -> libc::c_int {
    ...
    lua_setfield(state, -2, b"font\0" as *const u8 as *const libc::c_char);
    1
}
