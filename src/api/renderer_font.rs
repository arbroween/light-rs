...
unsafe extern "C" fn f_load(state: *mut lua_State) -> c_int {
    ...
    luaL_setmetatable(state, b"Font\0" as *const u8 as *const c_char);
    *self_0 = ren_load_font(filename, size);
    if (*self_0).is_null() {
        luaL_error(
            state,
            b"failed to load font\0" as *const u8 as *const c_char,
        );
    }
    1
}
...
static mut LIB: [luaL_Reg; 6] = [
    ...
    luaL_Reg {
        name: b"load\0" as *const u8 as *const c_char,
        func: Some(f_load),
    },
    luaL_Reg {
        name: b"set_tab_width\0" as *const u8 as *const c_char,
        func: Some(f_set_tab_width),
    },
    luaL_Reg {
        name: b"get_width\0" as *const u8 as *const c_char,
        func: Some(f_get_width),
    },
    luaL_Reg {
        name: b"get_height\0" as *const u8 as *const c_char,
...
pub unsafe extern "C" fn luaopen_renderer_font(state: *mut lua_State) -> c_int {
    luaL_newmetatable(state, b"Font\0" as *const u8 as *const c_char);
    luaL_setfuncs(state, LIB.as_ptr(), 0);
    lua_pushvalue(state, -1);
    lua_setfield(state, -2, b"__index\0" as *const u8 as *const c_char);
    1
}
